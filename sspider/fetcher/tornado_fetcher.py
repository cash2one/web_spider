...
from sspider.libs.url import quote_chinese
...
class Fetcher(object):
    user_agent = "webspider/%s " % __version__
    default_options = {
        'method': 'GET',
        'headers': {
        },
    ...
    @gen.coroutine
    def async_fetch(self, task, callback):
        '''异步Do one fetch'''
        url = task.get('url', '')
        if callback is None:
        ...
        try:
            if task.get('fetch', {}).get('fetch_type') in ('js', 'phantomjs'):
                type = 'phantomjs'
                result = yield self.phantomjs_fetch(url, task)#生成器的send方法给yield赋值
            else:
                type = 'http'
    ...
    def sync_fetch(self, task):
        '''同步Synchronization fetch, usually used in xmlrpc thread'''
