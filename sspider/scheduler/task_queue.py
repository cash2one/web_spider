...
class TaskQueue(object):
    ...
    def __init__(self, rate=0, burst=0):
        ...
        self.priority_queue = PriorityTaskQueue()#优先队列 – 任务已经到达执行时间，任务会根据优先级 priority ，在有流量配额情况下，依次发起调度
        self.time_queue = PriorityTaskQueue()#时间队列 – 任务尚未到达执行时间，时间拉长重试也在这个队列中
